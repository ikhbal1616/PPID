...
<head>
    ...
    <title>Kelola Dokumen | Admin PPID Universitas Baiturrahmah</title>
    <!-- CSRF Token -->
...
<body class="bg-slate-100 text-slate-800 font-sans antialiased overflow-x-hidden">
    ...
    <div class="flex min-h-screen">
        ...
        <aside id="admin-sidebar" class="w-64 bg-brand-green-950 text-slate-300 flex-shrink-0 flex flex-col justify-between transition-all duration-300 z-30 fixed top-0 bottom-0 left-0 -translate-x-full lg:translate-x-0 h-screen shadow-2xl">
            <div>
                ...
                <nav class="p-4 space-y-1.5 text-xs font-semibold">
                    ...
                    <!-- Fitur Baru: Kelola Dokumen (Active Style) -->
                    <a href="/admin/dokumen" class="flex items-center space-x-3 bg-brand-green-900/60 text-brand-gold-500 border-l-4 border-brand-gold-500 px-3 py-3 rounded-lg transition-all">
        ...
        <div class="flex-1 flex flex-col min-h-screen pl-0 lg:pl-64 transition-all duration-300 overflow-y-auto">
            ...
            <main class="p-6 space-y-6">
                ...
                <!-- HEADER SUMMARY -->
                <div>
                    <h1 class="text-2xl font-bold font-display tracking-tight text-slate-900 leading-tight">Kelola Dokumen PPID</h1>
                ...
                <!-- Alert Messages replaced with Modal Success Popup -->
                @if (session('success'))
                <div id="success-save-modal" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
                    ...
                    <div class="relative transform overflow-hidden rounded-3xl bg-gradient-to-br from-brand-green-900 to-brand-green-950 border border-brand-green-800/50 text-center shadow-2xl p-8 max-w-sm w-full transition-all duration-300 scale-100 opacity-100 z-10 flex flex-col items-center">
                        ...
                        <p class="text-[11px] text-emerald-100/75 mt-2.5 mb-6 leading-relaxed max-w-xs">
                            Perubahan pada kelompok dokumen, berkas lampiran, dan link URL telah berhasil diperbarui ke database sistem secara permanen.
                        </p>
                ...
                <!-- SAVE ALL FORM -->
                <form action="{{ route('admin.dokumen.update') }}" method="POST" id="main-submit-form" class="hidden">
                    @csrf
                    <!-- Hidden input to store JSON list values -->
                    <input type="hidden" name="dokumen_list" id="dokumen_list_input">
                </form>
                ...
                <div class="space-y-6">
                    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-6 space-y-6">
                        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                            <h3 class="font-bold text-slate-900 text-sm uppercase tracking-wider">Daftar Kelompok Dokumen</h3>
            ...
            <div class="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all w-full max-w-2xl mx-4 sm:mx-auto border border-slate-100 scale-95 opacity-0 duration-300 ease-out" id="kelompok-modal-container">
                ...
                <div class="p-6 space-y-4">
                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Judul Kelompok Dokumen (Accordion)</label>
                        <input type="text" id="kel-title" placeholder="Contoh: Rencana Strategis (Renstra) PPID..." class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Kategori Kelompok</label>
                            <select id="kel-category" class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tipe Badge</label>
                            <input type="text" id="kel-badge" placeholder="Contoh: Rencana, Laporan, SOP..." class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                        </div>
                    </div>

                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Deskripsi Singkat</label>
                        <textarea id="kel-desc" rows="3" placeholder="Tulis deskripsi singkat mengenai isi dokumen ini..." class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none resize-none"></textarea>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Link URL Tautan Langsung (Opsional)</label>
                            <input type="text" id="kel-link-url" placeholder="Contoh: https://example.com/portal-dokumen" class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                        </div>
                        <div class="space-y-1.5">
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Label Link Tautan (Opsional)</label>
                            <input type="text" id="kel-link-label" placeholder="Contoh: Kunjungi Portal" class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                        </div>
                    </div>

                    <div class="pt-4 border-t border-slate-100 flex justify-end space-x-2">
                        <button type="button" onclick="closeKelompokModal()" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all active:scale-95 cursor-pointer">Batal</button>
                        <button type="button" onclick="saveKelompokItem()" class="px-5 py-2.5 bg-brand-green-600 hover:bg-brand-green-700 text-white text-xs font-bold rounded-xl transition-all active:scale-95 shadow-md cursor-pointer" id="save-kel-btn-text">Simpan Kelompok</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- MODAL FORM TAMBAH/EDIT FILE SUB-ITEM -->
    <div id="file-modal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <!-- Scrollable content helper -->
        <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all w-full max-w-xl mx-4 sm:mx-auto border border-slate-100 scale-95 opacity-0 duration-300 ease-out" id="file-modal-container">
                <!-- Header -->
                <div class="relative bg-gradient-to-r from-brand-green-900 to-brand-green-950 px-6 py-4 flex justify-between items-center text-white">
                    <div>
                        <h3 class="text-sm font-bold font-display uppercase tracking-wider" id="form-file-title">Tambah File ke Kelompok</h3>
                        <p class="text-[9px] text-brand-gold-500 tracking-widest uppercase font-semibold mt-0.5" id="active-kel-display-title">Kelompok Title</p>
                    </div>
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nama/Judul Berkas</label>
                        <input type="text" id="file-title" placeholder="Contoh: Rencana Strategis Tahun 2025..." class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                    </div>

                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tipe Tautan Berkas</label>
                        <select id="file-type" onchange="toggleFileTypeInput(this.value)" class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                            <option value="">Pilih Tipe Tautan...</option>
                            <option value="PDF">Unggah File PDF</option>
                            <option value="Link">Tautan URL Eksternal / Halaman Internal</option>
                        </select>
                    </div>

                    <!-- PDF Upload Dropzone -->
                    <div class="space-y-1.5 hidden animate-fade-in" id="file-pdf-upload-container">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Unggah File PDF (Max 10MB)</label>
                        <div class="flex items-center space-x-3 bg-slate-50 p-4 border border-slate-200 border-dashed rounded-xl">
                            <div class="relative cursor-pointer bg-brand-green-600 hover:bg-brand-green-700 text-white font-bold text-xs px-4 py-2.5 rounded-lg transition-all active:scale-95 shadow-sm">
                                <span>Pilih Berkas PDF</span>
                                <input type="file" id="file-pdf-uploader" accept="application/pdf" onchange="uploadBerkasFile(this)" class="absolute inset-0 opacity-0 cursor-pointer w-full h-full">
                            </div>
                    ...
                    <!-- External URL Input -->
                    <div class="space-y-1.5 hidden animate-fade-in" id="file-url-container">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Link URL / Path Halaman</label>
                        <input type="text" id="file-url" placeholder="Contoh: https://example.com/dokumen.pdf atau /profil" class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-3 rounded-xl transition-all outline-none">
                    </div>

                    <div class="pt-4 border-t border-slate-100 flex justify-end space-x-2">
                        <button type="button" onclick="closeFileFormPanel()" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all active:scale-95 cursor-pointer">Batal</button>
                        <button type="button" onclick="saveFileItem()" class="px-5 py-2.5 bg-brand-green-600 hover:bg-brand-green-700 text-white text-xs font-bold rounded-xl transition-all active:scale-95 shadow-md cursor-pointer" id="save-file-btn-text">Tambahkan Berkas</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        if (liveDateEl) {
            ...
            liveDateEl.innerText = new Date().toLocaleDateString('id-ID', dateOptions);
        }
        ...
        try {
            const rawData = {!! json_encode(setting('dokumen_list')) !!};
            if (rawData) {
        ...
        function renderKelompokList() {
            ...
            const input = document.getElementById('dokumen_list_input');
            ...
            if (kelompokItems.length === 0) {
                container.innerHTML = `
                    <div class="p-8 text-center text-xs text-slate-400 font-medium bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                        Belum ada Kelompok Dokumen yang ditambahkan. Silakan klik tombol "Tambah Kelompok".
                    </div>
                `;
                return;
            }
        ...
        function saveKelompokItem() {
            ...
            const category = document.getElementById('kel-category').value;
            const badge_type = document.getElementById('kel-badge').value.trim();
            ...
            if (!title) {
                showErrorModalPopup('Gagal Menyimpan', 'Judul Kelompok Dokumen wajib diisi.');
                return;
            }
            ...
            if (editingKelompokIndex !== null) {
                ...
                closeKelompokModal();
                renderKelompokList();
                saveToServerAsync("Berhasil Diperbarui", "Kelompok dokumen telah berhasil diperbarui di database.");
            } else {
                kelompokItems.push({
                    title,
                    category,
                ...
                closeKelompokModal();
                renderKelompokList();
                saveToServerAsync("Berhasil Ditambahkan", "Kelompok dokumen baru telah berhasil disimpan ke database.");
            }
        }
        ...
        function editKelompokItem(index) {
            ...
            document.getElementById('kel-category').value = item.category || 'profil';
            document.getElementById('kel-badge').value = item.badge_type || '';
        ...
        function cancelKelompokEdit() {
            ...
            document.getElementById('kel-category').value = 'profil';
            document.getElementById('kel-badge').value = '';
        ...
        function deleteKelompokItem(index) {
            if (confirm('Apakah Anda yakin ingin menghapus kelompok ini beserta seluruh berkas di dalamnya?')) {
                ...
                closeFileFormPanel();
                renderKelompokList();
                saveToServerAsync("Berhasil Dihapus", "Kelompok dokumen telah berhasil dihapus dari database.");
            }
        }
