...
@section('title', 'Kelola Informasi Tersedia Setiap Saat')
...
<!-- HEADER SUMMARY -->
                <div>
                    <h1 class="text-2xl font-bold font-display tracking-tight text-slate-900 leading-tight">Kelola Informasi Tersedia Setiap Saat</h1>
                ...
                <form action="{{ route('admin.informasi-tersedia-setiap-saat.update') }}" method="POST" id="main-submit-form" class="hidden">
                    @csrf
                    <!-- Hidden input to store JSON list values -->
                    <input type="hidden" name="informasi_setiap_saat_list" id="informasi_setiap_saat_list_input">
                </form>
                    ...
                    <div class="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl flex flex-col justify-between transform scale-95 opacity-0 transition-all duration-300 ease-out border border-slate-100 overflow-hidden z-10" id="kelompok-modal-container">
                        ...
                        <div class="bg-gradient-to-r from-brand-green-900 to-brand-green-950 px-6 py-5 flex justify-between items-center text-white">
                            <div>
                                ...
                                <p class="text-[9px] text-brand-gold-500 tracking-wider uppercase font-semibold mt-0.5">Kelompok Informasi PPID</p>
                            </div>
                        ...
                        <div class="p-6 space-y-4 text-xs text-slate-700">
                            <div class="space-y-1.5">
                                ...
                                <input type="text" id="kel-title" class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-2.5 rounded-xl transition-all outline-none" required placeholder="Masukkan judul kelompok informasi...">
                            </div>

                            <div class="space-y-1.5">
                                <label for="kel-desc" class="block font-bold text-slate-600 uppercase tracking-wider text-[10px]">Deskripsi Singkat</label>
                                <textarea id="kel-desc" rows="2" class="w-full bg-slate-50 border border-slate-200 focus:border-brand-green-500 focus:bg-white text-slate-800 text-xs px-3.5 py-2.5 rounded-xl transition-all outline-none resize-none" placeholder="Masukkan deskripsi mengenai kelompok informasi ini..."></textarea>
                            </div>
...
<script>
        ...
        try {
            const rawData = {!! json_encode(setting('informasi_setiap_saat_list')) !!};
            if (rawData) {
        ...
        function renderKelompokList() {
            ...
            const input = document.getElementById('informasi_setiap_saat_list_input');
